 ...
 *       再实现 {@link #listFormFields} 返回字段元数据。流程发布时前端会把字段快照带到模型 JSON 的 metaFields 里，
 ...
 * <p>默认实现返回空列表。设计器没有表单可选，但流程模型 JSON 里已经自带 metaFields 快照，
...
public interface FlowDesignerFormProvider {
     ...
     * 流程发布时前端会直接把字段快照写入模型 JSON 的 metaFields，后续条件分支和表单渲染都从流程模型 JSON 读取。</p>
     *
     * @param formKey 表单编码
