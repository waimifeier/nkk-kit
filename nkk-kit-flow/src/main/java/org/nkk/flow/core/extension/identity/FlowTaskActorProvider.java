...
public interface FlowTaskActorProvider {
     ...
     * 其次调用 {@link #getDynamicAssignee(FlowNodeModel, FlowExecution)}，最后回退到节点模型的
     * {@link FlowNodeModel#getNodeAssigneeList()}。</p>
     *
     * <p>如果使用方需要根据用户、角色、部门、岗位、表单字段等业务组织关系动态计算办理人，
     * 可以覆盖该方法并返回最终写入任务表的参与者快照。</p>
     *
     * @param nodeModel 当前正在创建任务的流程节点
     * @param execution 当前流程执行上下文，可获取发起人、实例、变量、当前任务等信息
     * @return 任务参与者列表；返回 {@code null} 或空集合时，引擎会调用
     * {@link #abnormal(FlowTask, List, FlowExecution, FlowNodeModel)} 处理异常场景
     */
    List<FlowTaskActor> getTaskActors(FlowNodeModel nodeModel, FlowExecution execution);
     ...
     * @return 节点参与人列表
     */
    default List<FlowNodeAssignee> getNodeAssignees(FlowNodeModel nodeModel, FlowExecution execution) {
     ...
     */
    default FlowDynamicAssignee getDynamicAssignee(FlowNodeModel nodeModel, FlowExecution execution) {
        return null;
    }
     ...
     */
    default boolean abnormal(FlowTask task, List<FlowTaskActor> taskActors, FlowExecution execution, FlowNodeModel nodeModel) {
     ...
     * 0 用户，1 角色，2 部门。默认实现中，角色/部门节点在非全员参与策略下会分别返回角色/部门，
     * 其他情况返回用户。</p>
     *
     * @param nodeModel 当前节点模型
     * @return 参与者类型
     */
    Integer getActorType(FlowNodeModel nodeModel);
}
